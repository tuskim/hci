...
function f_Save(){
	...
	for( var i=1; i <= ds_input.CountRow; i++ ) {
		if(ds_input.NameValue(i, "materCd") == ""){
			alert("<%=source.getMessage("dev.warn.com.required","Material")%>");
			gr_input.Focus();
			return;
		}
...
function f_Enable(){
	...
	gr_input.ColumnProp("materNm", "Edit") = "";
...
function f_Disable() {
	...
	gr_input.ColumnProp("materNm", "Edit") = "none";
...
<script language=Javascript for=gr_output event=OnClick(row,colid)>
	...
	if(status == "00" && row > 2){
		...
		gr_output.ColumnProp("materNm", "Edit") = "";
		gr_output.ColumnProp("storLoct", "Edit") = "";
	}else{
		gr_output.ColumnProp("materNm", "Edit") = "none";
		gr_output.ColumnProp("materNm", "Edit") = "none";
...
<script language=javascript for=gr_input event=OnPopup(Row,Colid,data)>
	if ( Colid == "materCd" || Colid == "materNm") {
 		openCementMaterialCodeListPopUp(Row, ds_input,"SD");
	}
</script>
