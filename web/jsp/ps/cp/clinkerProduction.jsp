...
/*
 *********************************************************************************************
 * @source  : clinkerProduction.jsp
 * @desc    : Clinker Production
 *--------------------------------------------------------------------------------------------
 * VER  DATE         AUTHOR      DESCRIPTION
 * ---  -----------  ----------  -------------------------------------------------------------
 * 1.0  2016/08/02   kjkim       Init
 * ---  -----------  ----------  -------------------------------------------------------------
 * PT-PAM System.
...
<title>Clinker Production</title>
...
var g_mstRowPosition = 1;   //Clinker Production List Row Position
//-------------------------------------------------------------------------
//Page Init
//-------------------------------------------------------------------------
function f_Init() {
...
function f_Retrieve() {
    ...
    ds_master.DataID = "ps.cp.retrieveClinkerProductionList.gau"+condition;
    ds_master.Reset();
...
function f_New(){
  ...
  ds_masterTmp.DataID = "ps.cp.retrieveClinkerProductionAddRow.gau"+condition;
  ds_masterTmp.Reset();
  ...
  ds_master.NameValue(ds_master.RowPosition, "clinkerQty") = ds_masterTmp.NameValue(1, "clinkerQty");
  ...
  ds_outputTmp.DataID = "ps.cp.retrieveClinkerProductionAddRow.gau"+condition;
  ds_outputTmp.Reset();
  ...
  //ds_input Add Row
    ...
    condition += "companyCd=<%=g_companyCd %>";
  	...
  	ds_inputTmp.DataID = "ps.cp.retrieveClinkerProductionAddRow.gau"+condition;
  	ds_inputTmp.Reset();
...
function f_Delete(){
	...
	if(confirm("<%=msgDelete%>")){
		...
		g_mstRowPosition = ds_master.RowPosition;  //Clinker Production List Row Position
		
		tr_cudDataDelete.KeyValue = "Servlet(I:IN_DS1=ds_master)";
		tr_cudDataDelete.Action   = "/ps.cp.cudClinkerProductionDel.gau";
...
function f_Save(){
	...
	var clinkerQty = ds_input.NameValue(ds_input.RowPosition, "prodInQty");
	if(clinkerQty == 0){
		alert("<%=source.getMessage("dev.warn.com.inputQty","Clinker")%>");
		return;
	}
   ...
   if(confirm('<%=msgSave%>')){
    ...
    tr_cudDataSave.Action   = "/ps.cp.cudClinkerProductionReg.gau";
    tr_cudDataSave.post();
...
function f_Approval(){
	...
	var clinkerQty = ds_input.NameValue(ds_input.RowPosition, "prodInQty");
	if(clinkerQty == 0){
		alert("Input Material for Clinker Qty.");
		return;
	}
	...
	if(confirm("<%=msgApproval%>")){
		...
		tr_cudDataApproval.Action   = "/ps.cp.cudClinkerProductionApproval.gau";
...
function f_Reject(){
	...
	if(confirm("<%=msgReject%>")){
		...
		tr_cudDataReject.Action   = "/ps.cp.cudClinkerProductionReject.gau";
...
function f_sapSend(){
	...
	if(f_validationBeforeProductionStatus()  == true){
		var msg = "<%=source.getMessage("dev.warn.com.checkRawMix")%>";
		msg += ds_master.NameValue(ds_master.RowPosition,"prodDate")
		alert(msg);
		return;		
	}
	...
	if(confirm("<%=msgSAPsend%>")){
		...
		tr_sapSend.Action   = "/ps.cp.cudClinkerProductionSAPsend.gau";
...
function f_sapCancel(){
	...
	if(confirm("<%=msgSAPcancel%>")){
		...
		tr_sapCancel.Action   = "/ps.cp.cudClinkerProductionSAPcancel.gau";
...
function f_excelDown(){
    ...
    if (checkData()) {
        ...
        ds_excelDown.DataID = "ps.cp.retrieveClinkerProductionExcelList.gau"+condition;
        ds_excelDown.Reset();
...
//-------------------------------------------------------------------------
function f_excel() {
	gf_excel2(ds_excelDown,gr_excelDown,"<%= currentDate %>" ,"Clinker Production Report","c:\\");
}
...
function f_validationProdDate() {
	...
	v_url.SetPage("ps.cp.retrieveClinkerProductionList.gau");
...
<!-- Grid 용 Clinker Production List DataSet-->
...
<!-- Grid 용 Use of Material for Clinker DataSet-->
...
<!-- Grid 용 Result of Clinker DataSet-->
...
------------------------------------------------------------------------------>
<!-- Clinker Production List DataSet Event[S] -->
...
<script language=javascript for=ds_master event=OnRowPosChanged(row)>
  ...
  if(row>0 && ds_master.RowStatus(row)!=1 ){
    ...
    ds_output.DataID = "/ps.cp.retrieveClinkerProductionMaterial.gau?"+param;
    ds_output.Reset();

    ds_input.DataID = "/ps.cp.retrieveClinkerProductionResult.gau?"+param;
    ds_input.Reset();
...
<!-- Clinker Production List DataSet Event[E] -->


<!-- Use of Material for Clinker DataSet Event[S] -->
...
<script language=javascript for=ds_output event=OnColumnChanged(row,colid)>
  ...
  ds_input.NameValue(1, "prodInQty") = (cnt/1.65); // Quantity : (prodOutQty)/1.65

</script>

<script language=JavaScript for=ds_output event=OnLoadError()>
</script>
<!-- Use of Material for Clinker DataSet Event[E] -->


<!-- Result of Clinker Production DataSet Event[S] -->
...
<script language=javascript for=ds_input event=OnColumnChanged(row,colid)>
  ...
  for( var i=1; i <= ds_master.CountRow; i++){
	  ...
	  if(ds_master.NameValue(i, "syskey") == productNo){
		  ds_master.NameValue(i, "clinkerQty") = prodInQty;
	  }
  }
</script>
...
<!-- Result of Clinker Production DataSet Event[E] -->
...
<div id="conts_box">
  <h2> <span>Clinker Production</span></h2>
  ...
  <!-- 그리드 S -->
    <div>
      <object id="gr_master" classid="<%=LGauceId.GRID %>" style="width:100%;height:175px;" class="comn"
          dataName="Clinker Production List
        ...
        <param name='Format'
               value="
                   ...
                   <C> id='clinkerQty'  name='Production;Quantity(KG)'                            align='right'    width='120' Edit='none' show='true'      DisplayFormat ='#,###.000' sumtext='@sum' sumbgcolor='#ECE6DE' sumcolor='#666666'</C>
  ...
  <div class="sub_stitle">
    <p>Use of Material for Clinker</p>
  </div>
    <!-- 검색 S -->
  ...
  <!-- 그리드 E -->
    ...
    <div class="sub_stitle"> <p>Result of Clinker</p></div>

  <!-- 그리드 S -->
    <div>
      <object id="gr_input" classid="<%=LGauceId.GRID %>" style="width:100%;height:60px;" class="comn">
			...
			<param name="Format"
				value="
                   ...
                   <G> name = 'Use of Material'
                     <C> id='rawmix'    name='RawMix'                             align='right'    width='120' Edit='none' show='true'      sumtext='@sum' sumbgcolor='#ECE6DE' sumcolor='#666666'</C>
                     <C> id='coal'      name='Coal'                               align='right'    width='120' Edit='none' show='true'      sumtext='@sum' sumbgcolor='#ECE6DE' sumcolor='#666666'</C>
                   </G>
                   
                   <G> name = 'Result of Production'
                     <C> id='clinkerQty'  name='Clinker'                          align='right'    width='160' Edit='none' show='true'      DisplayFormat ='#,###.000'  sumtext='@sum' sumbgcolor='#ECE6DE' sumcolor='#666666'</C>
                   </G>
