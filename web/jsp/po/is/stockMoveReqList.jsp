...
//-------------------------------------------------------------------------		
function init(){
	for(var i=1; i<=ds_issuelocation.CountRow; i++){
		if(ds_issuelocation.NameValue(i, "code") == ds_defaultDeptCd.NameValue(1, "deptCd")){
			issueLoc.Index = i-1;
			break;
		}
	}
...
function f_delete() {
	...
	for(var i=1; i<=ds_stockMoveMst.CountRow; i++){
		if(ds_stockMoveMst.NameValue(i, "chk") =="T"){
			isChecked = true;
			
			// status가 다를 경우
			if ( ds_stockMoveMst.nameValue(i,"status") != '01' ) {
...
/* 
    posting Date 와 현재 날짜를 비교하는건 의미가 없음
	...
	*/
	...
	if(confirm("<%= msgCancel %>")){
		...
		tr_master.Action   = "po.is.cudStockMoveReqDelete.gau";
		tr_master.post();
	}
				...
				<dt> 
					<table width="100%" border="0" cellpadding="0" cellspacing="0" class="output_board2" >
						...
						<tr>
							...
							<td>
								<object id="receLoc"  classid="<%=LGauceId.LUXECOMBO%>" align="absmiddle" width="140">
						...
						<tr>
							...
							<td>
								<input type="text" id="orderDateFrom" value="<%= prevDate %>" 	 onblur="valiDate(this);"  style="width:65px;"/>&nbsp;<img src="<%= images %>button/cal_icon.gif" alt="Calendar Icon" onClick="javascript:OpenCalendar(orderDateFrom);" style="cursor:hand"/> ~ 
								<input type="text" id="orderDateTo"   value="<%= currentDate %>" onblur="valiDate(this);"  style="width:65px;"/>&nbsp;<img src="<%= images %>button/cal_icon.gif" alt="Calendar Icon" onClick="javascript:OpenCalendar(orderDateTo);" style="cursor:hand"/>
							</td>
							<th><%= columnData.getString("status") %></th>
							<td>
								<object id="progStatus"  classid="<%=LGauceId.LUXECOMBO%>" align="absmiddle" width="140">
              ...
              <td width="100%"><comment id="__NSID__"><object id="gr_stockMoveMst" classid="<%=LGauceId.GRID%>" class="comn" width="100%"	height="180px" class="B_list">
				...
				<param name="Format"
					value='
			            ...
			            <C>id="tranNo"    		name="<%=columnData.getString("tran_no") %>"    		Edit="none"   	align="center"  	width="100"   show="true"</C>
			            <C>id="issueLocNm"  	name="<%=columnData.getString("issue_loc") %>" 	    	Edit="none"		align="left"  		width="140"  show="true"</C> 
			            <C>id="receLocNm"   	name="Receipt Location"	    	Edit="none"  	align="left"    	width="140"  show="true"</C>
			            <C>id="moveDate" 		name="<%=columnData.getString("posting_date") %>"   	Edit="none"   	align="center"  	width="110"   show="true" Mask="XXXX/XX/XX" </C>
			            <C>id="statusNm"    	name="<%=columnData.getString("status") %>"       		Edit="none"   	align="left"  		width="140"   show="true"</C>
			            <C>id="status"    		name="<%=columnData.getString("status") %>"     show="false"  		Edit="none"   	align="left"  		width="110"   </C>
			            <C>id="requisitionStatusNm"    	name="<%=columnData.getString("requisition_status") %>"     show="false"  		Edit="none"   	align="left"  		width="110"   </C>
			            <C>id="delFlag"    	name="<%=btnDel%>"       		Edit="none"   	align="center"  		width="50"   show="false"</C>
				'>
              ...
              <td width="100%"><comment id="__NSID__"><object id="gr_stockMoveDtl" classid="<%=LGauceId.GRID%>" class="comn" width="100%"	height="140px" class="B_list">
				...
				<param name="Format"
					value='
			            ...
			            <C>id="tranQty"    	name="<%=columnData.getString("receipt_qty") %>"       Edit="none"   	align="right"  		width="110"  DisplayFormat ="#,###.000" </C>
		...
		<!-- </div>	 -->
		 <!-- 버튼 S -->	
          <div id="btn_area">
                ...
                <input type="button" onfocus="blur();" onmouseover="this.style.color='#cd1950'" onmouseout="this.style.color='#7d7f84'" value="<%=btnDel%>" onclick="javascript:f_delete();"/></span> 
            </p>
        </div>
        <!-- 버튼 E -->	
