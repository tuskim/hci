...
function f_Save() {
	...
	if (checkSaveData('SAVE')) {
	    ...
	    if(confirm("Are you sure to save?")){     // Are you sure to save?
			...
			tr_cudData.Action    = "po.oc.cudPurchReqMgnt.gau";
			tr_cudData.Post();
		}
...
function f_Delete(){
	...
	if (checkDelData()) {
		...
		var RowStatus = ds_purchReqMst.RowStatus(ds_purchReqMst.RowPosition);
		if ( RowStatus == 1 ) {
	    	ds_purchReqMst.DeleteRow(ds_purchReqMst.RowPosition);      // 신규 추가건만 삭제 가능
		...
		if (confirm('Are you sure to Delete?')) {
			...
			tr_cudData.Action   = "po.oc.cudDelPurchReqMgnt.gau";
    ...
    if (checkData()) {
		...
		var condition = "?";
	    	condition += "reqDeptCd=" + deptCd.ValueOfIndex("code",deptCd.Index);
	 		...
	 		//alert(condition);
		ds_purchReqMst.DataID = "po.oc.retrievePurchReqMstMgnt.gau"+condition;
...
function f_New(){ 
	...
	if(ds_purchReqMst.CountRow == 0)	{
        cfHideNoDataMsg(gr_purchReqMst);           // 최초 입력인 경우, 해당 건을 만들어주고 진행해야한다.
        ds_purchReqMst.ClearAll();
        ds_purchReqMst.setDataHeader( strHeader );
		cfSetSearchGridBtn(strHeader);
	}
	
    ds_purchReqMst.AddRow();
	ds_purchReqMst.Namevalue(ds_purchReqMst.RowPosition,"companyCd") = '<%= g_companyCd %>';
	ds_purchReqMst.Namevalue(ds_purchReqMst.RowPosition,"reqDeptCd") = deptCd.ValueOfIndex("code",deptCd.Index);
	ds_purchReqMst.Namevalue(ds_purchReqMst.RowPosition,"reqDeptNm") = deptCd.ValueOfIndex("name",deptCd.Index);
}
...
function f_Add(){
    ...
    if(ds_purchReqDtl.CountRow == 0) {
        cfHideNoDataMsg(gr_purchReqDtl);           // 최초 입력인 경우, 해당 건을 만들어주고 진행해야한다.
        ds_purchReqDtl.ClearAll();
        ds_purchReqDtl.setDataHeader( strBody );
		cfSetSearchGridBtn(strBody);
    }

    ds_purchReqDtl.AddRow();
	ds_purchReqDtl.Namevalue(ds_purchReqDtl.RowPosition,"companyCd") = ds_purchReqMst.Namevalue(ds_purchReqMst.RowPosition,"companyCd");
	ds_purchReqDtl.Namevalue(ds_purchReqDtl.RowPosition,"prNo")      = ds_purchReqMst.Namevalue(ds_purchReqMst.RowPosition,"prNo");
}
...
//-------------------------------------------------------------------------
function f_Del(){
    ds_purchReqDtl.ViewDeletedRow = "false";
    ds_purchReqDtl.DeleteRow(ds_purchReqDtl.RowPosition);      // 신규 추가건만 삭제 가능
}
...
<script language="javascript"  for=ds_purchReqMst event=OnRowPosChanged(row)>
	...
	if ( row > 0 ) {
		...
		ds_purchReqDtl.DataID = "po.oc.retrievePurchReqDtlMgnt.gau"+condition;
...
<div id="conts_box">
	...
	<object id="gr_purchReqMst" classid="<%=LGauceId.GRID %>" style="width:100%;height:130px;"
		...
		<param Name='Format'
			value='
                ...
                <C> id=budget           name="<%= columnData.getString("budget") %>"        align="rirht"   width="70"    Edit="none"   show="false"  </C>
                <C> id=idrAmt           name="<%= columnData.getString("idr_amt2") %>"        align="right"   width="80"    Edit="none"   show="true"   </C>
                ...
                <C> id=gmoConid        	name="<%= columnData.getString("gmo_conid") %>"     align="center"   width="70"    Edit="none"   show="false"  </C>
                <C> id=gmoCondate      	name="<%= columnData.getString("gmo_condate") %>"   align="center"   width="70"    Edit="none"   show="false"  </C>
                <C> id=headConid       	name="<%= columnData.getString("head_conid") %>"    align="center"   width="70"    Edit="none"   show="false"  </C>
                <C> id=headCondate     	name="<%= columnData.getString("head_condate") %>"  align="center"   width="70"    Edit="none"   show="false"  </C>
	      '>
	</object>
 	</div>
    ...
    <div id="output_board_area">
	    ...
	    <tr>
		  <th><%= columnData.getString("pr_no") %></th>
		  <td><input type="text" id="prNo" 	style="width:150px;" maxlength="10" readonly class="txtField_read"></td>
		  <th><%= columnData.getString("budget") %></th>
		  <td><input type="text" id="budget" 	style="width:150px;" maxlength="33" required></td>  
		</tr>
	    <tr>
		  <th><%= columnData.getString("req_dept_cd") %></th>
		  <td>
			<object id="reqDeptCd"  classid="<%=LGauceId.LUXECOMBO%>" align="absmiddle" width="100">
			    <param name="ComboDataID"       value="ds_deptCode">
				<param name="ListCount"     	value="10">
				<param name="BindColumn"    	value="code">
				<param name=SyncComboData       value="false">
		  ...
		  <th><%= columnData.getString("requester_nm") %></th>
		  <td><input type="Text" id="requester"  style="width:150px;" maxlength="33" required></td>  
		</tr>
	    <tr>
		  <th><%= columnData.getString("req_date") %></th>
		  <td><input type="text" id="reqDate" 	style="width:60px;" maxlength="10">&nbsp;<img src="<%= images %>button/cal_icon.gif" alt="Calendar Icon" onClick="javascript:OpenCalendar(reqDate);" style="cursor:hand"/></td>
		  <th><%= columnData.getString("deli_req_date2") %></th>
		  <td><input type="text" id="deliReqDate"   style="width:60px;" maxlength="10">&nbsp;<img src="<%= images %>button/cal_icon.gif" alt="Calendar Icon" onClick="javascript:OpenCalendar(deliReqDate);" style="cursor:hand"/></td>  
		</tr>
	    <tr>
		  <th><%= columnData.getString("pur_comment") %></th>
		  <td colspan="3"><input type="text" id="purComment" style="width:200px;" maxlength=30></td>  
		</tr>
	  </table>
	</div>

	<object id=bd_purchMgnt classid="<%=LGauceId.BIND%>">
		<param name=DataID value=ds_purchReqMst>
		<param name=BindInfo 
			value='
                <C> Col=companyCd       Ctrl=companyCd      Param=value  </C>
                <C> Col=prNo            Ctrl=prNo           Param=value  </C>
                <C> Col=reqDeptCd       Ctrl=reqDeptCd      Param=BindColVal  </C>
                <C> Col=reqDeptNm       Ctrl=reqDeptNm      Param=value  </C>
                <C> Col=requester       Ctrl=requester      Param=value  </C>
                <C> Col=requesterNm     Ctrl=requesterNm    Param=value  </C>
                <C> Col=purComment      Ctrl=purComment     Param=value  </C>
                <C> Col=reqDate         Ctrl=reqDate        Param=value  </C>
                <C> Col=deliReqDate     Ctrl=deliReqDate    Param=value  </C>
                <C> Col=budget          Ctrl=budget         Param=value  </C>
                <C> Col=amount          Ctrl=amount         Param=value  </C>
                <C> Col=purDeptCd       Ctrl=purDeptCd      Param=value  </C>
                <C> Col=gmoConid        Ctrl=gmoConid       Param=value  </C>
                <C> Col=gmoCondate      Ctrl=gmoCondate     Param=value  </C>
                <C> Col=headConid       Ctrl=headConid      Param=value  </C>
                <C> Col=headCondate     Ctrl=headCondate    Param=value  </C>
	  		'>
	</object>
