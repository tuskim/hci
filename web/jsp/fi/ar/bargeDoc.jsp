...
function f_Retrieve() {
    var condition = "?";
		    condition += "flag="                			+ "2";
		    condition += "&syskey="                			+ document.all.syskey.value;
		    condition += "&mvCd="               	+ document.all.mvCd.value;    
		    condition += "&postDateFrom="   	+ document.all.postDateFrom .value;  
		    condition += "&postDateTo="       	+ document.all.postDateTo.value;
		    condition += "&bargeSeq="              		+ document.all.bargeSeq.value;
		    condition += "&bargeCd="            	+ document.all.bargeCd.value;
		    condition += "&mvArrivalDateFrom="   	+ document.all.mvArrivalDateFrom.value;   
		    condition += "&mvArrivalDateTo="        	+ document.all.mvArrivalDateTo.value;
	 		//alert(condition);
...
function f_excelDown(){
		...
		gr_costTotMst.SetExcelTitle(1, "value:Barge Doc List; font-face:±¼¸²u; font-size:30pt; font-color:black;font-bold; font-underline; bgcolor:white; align:center; line-color:red;line-width:2pt; skiprow:1;");
		...
		gr_costTotMst.GridToExcel("Barge_Doc_List", "C:\\Barge_Doc_List.xls", lOption);
...
<div id="conts_box">
			...
			<div id="output_board_area">
				<table width="100%" border="0" cellpadding="0" cellspacing="0" class="output_board2" >
					<colgroup>
   					...
   					<tr>
  						<th><%= columnData.getString("syskey2") %></th>
       					<td>
							<input type="text" id="syskey" 	style="width:70px;" maxlength="60">
 						</td>
						<th><%= columnData.getString("mv_Cd") %></th>
						<td>
							<input type="text" id="mvCd" 	style="width:70px;" maxlength="60">
						</td>
   					...
   					<tr>
  						<th><%= columnData.getString("barge_Seq") %></th>
       					<td>
							<input type="text" id="bargeSeq" 	style="width:70px;" maxlength="70">
						</td>		
  						<th><%= columnData.getString("barge_Cd") %></th>
       					<td>
							<input type="text" id="bargeCd" 	style="width:70px;" maxlength="70">
						</td>		
						<th><%= columnData.getString("mv_Arrival_Date") %></th>
       					<td>
								<input type="text" id="mvArrivalDateFrom" value="<%= prevDate %>" 		onblur="valiDate(this);" style="width:65px;"/>&nbsp;<img src="<%= images %>button/cal_icon.gif" alt="Calendar Icon" onClick="javascript:OpenCalendar(mvArrivalDateFrom);" style="cursor:hand"/> ~ 
								<input type="text" id="mvArrivalDateTo"   value="<%= currentDate %>" 	onblur="valiDate(this);" style="width:65px;"/>&nbsp;<img src="<%= images %>button/cal_icon.gif" alt="Calendar Icon" onClick="javascript:OpenCalendar(mvArrivalDateTo);" style="cursor:hand"/>
	  					</td>	
					</tr>
  				</table>
			</div>
	...
	<object id="gr_costTotMst" classid="<%=LGauceId.GRID %>" style="width:100%;height:370px;" class="comn"
		...
		<param Name='Format'
			value='
                ...
                <C> id=syskey 				name="<%= columnData.getString("syskey") %>"      		 	align="center"   width="90"    Edit="none"   show="true"  suppress=3</C>
                <C> id=mvCd   				name="<%= columnData.getString("mv_Cd") %>"     	 			align="center"   width="78"   Edit="none"   show="true"  suppress=3</C>
                <C> id=bargeSeq  			name="<%= columnData.getString("barge_Seq") %>" 			align="center"   width="80"   Edit="none"   show="true"  suppress=3</C>
                <C> id=bargeCd  			name="<%= columnData.getString("barge_Cd") %>"     	 	align="center"   width="80"    Edit="none"   show="true"  suppress=3</C>
                <C> id=bargePostDate  name="<%= columnData.getString("post_Date") %>"     	      align="center"   width="80"    Edit="none"   show="true"  Mask="XXXX/XX/XX"  suppress=3</C>
                <C> id=mvArrivalDate   name="<%= columnData.getString("mv_Arrival_Date") %>" align="center"   width="80"    Edit="none"   show="true"  Mask="XXXX/XX/XX"  suppress=3</C>
                <C> id=loadingQty 			name="<%= columnData.getString("loading_Qty") %>"       	  align="right"   width="115"    Edit="none"   show="true"  suppress=3</C>
                
                <C> id=seq2         			name="<%= columnData.getString("seq") %>"     					align="center"     width="30"    Edit="none"   show="true" </C>
                <C> id=companyCd   		name="<%= columnData.getString("company_Cd") %>"     	align="center"   width="80"    Edit="none"   show="false"</C>
                <C> id=deptCd            	name="<%= columnData.getString("dept_Cd") %>"     		 	align="center"   width="60"    Edit="none"   show="false" </C>
                <C> id=docYm      			name="<%= columnData.getString("doc_Ym") %>"     		 	align="center"   width="55"    Edit="none"   show="true"  Mask="XXXX/XX"</C>
                <C> id=docSeq      			name="<%= columnData.getString("doc_Seq") %>"     	     	align="center"     width="70"    Edit="none"   show="true"</C>
                <C> id=sapDocSeq       	name="<%= columnData.getString("sap_Doc_Seq") %>"     	align="center"   width="80"    Edit="none"   show="true" </C>
                ...
                <C> id=amount  				name="<%= columnData.getString("amount") %>"     	 		align="right"   width="100"    Edit="none"   show="true"    sumbgcolor="#ECE6DE" sumcolor="#666666" DisplayFormat ="#,###.00" </C>
                <C> id=currencyCd     	name="<%= columnData.getString("currency_Cd") %>"     	align="center"   width="45"    Edit="none"   show="true"  </C>
                <C> id=vendor  				name="<%= columnData.getString("vendor") %>"     	 			align="left"   width="130"    Edit="none"   show="true"  EditStyle="LookUp" 	Data="ds_vendor:Code:NAME" </C>
    ...
    <div>
				...
				<comment id="__NSID__"><object id="lc_vendor"  classid="<%=LGauceId.LUXECOMBO%>" width="0">
