...
var docNo = "<%=request.getParameter("docNo")%>" ? "<%=request.getParameter("docNo")%>" : -1; 
function f_OnLoad() {
...
<input type="hidden" name="docNo" value="<%=result.getString("docNo") %>"/>
        	...
        	<table width="100%" border="0" cellpadding="0" cellspacing="0" class="output_board" >
	           	<colgroup>
					...
					<th> <%=columnData.getString("attachment") %> </th>
						...
						&nbsp;<img src="<%=images%>btn_file01.gif" align="absmiddle" alt="file" onClick="" style="cursor:hand; margin-right:3px;"/> <a href="fi.ar.DownloadConDocFile.dev?filename=<%=URLEncoder.encode(fileAttach.getString("fileUrl", i),"UTF-8")%>&fileReal=<%=URLEncoder.encode(fileAttach.getString("fileNm", i),"UTF-8")%>"> <%=fileAttach.getString("fileNm",i) %></a>
					    <br>
						<%} %>
 			 			</td>                             
                	</tr>
