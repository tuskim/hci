...
public class RetrieveMovingQtyCmd implements LGauceCommandIF {
    ...
    public void execute(HttpServletRequest req, HttpServletResponse res, GauceRequest gauceRequest, GauceResponse gauceResponse) throws Exception {
	    ...
	    inputData.setString("issueLoct", 	inputData.getString("issueLoct"));
	    inputData.setString("materCd", 	inputData.getString("materCd"));
	    
	    ReceiptMgntBiz biz = new ReceiptMgntBiz();
		LMultiData result = biz.retrieveMovingMaterialQty(inputData);
		
        
		LLog.info.write("\n RetrieveMovingQtyCmd result:" + result.toString());		
		//req.setAttribute("resultData", result);

	    LGauceConverter.extractToGauceDataSet(result, gds);
	    gds.flush();
	    
		LLog.info.write("\n RetrieveMovingQtyCmd --> End \n");		
