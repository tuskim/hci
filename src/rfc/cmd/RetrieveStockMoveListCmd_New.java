...
public class RetrieveStockMoveListCmd_New implements LGauceCommandIF{
	...
	public void execute(HttpServletRequest req, HttpServletResponse res, GauceRequest gauceRequest, GauceResponse gauceResponse) throws Exception {
        ...
        gds.addDataColumn( new GauceDataColumn( "remainQty" 	, GauceDataColumn.TB_STRING  , 20  ) ); // 재고수량
        gds.addDataColumn( new GauceDataColumn( "movingQty" 	, GauceDataColumn.TB_DECIMAL  , 15, 3  ) ); // 출고수량
        gds.addDataColumn( new GauceDataColumn( "tranQty" , GauceDataColumn.TB_DECIMAL  , 15, 3  ) ); // 출고잔량
		...
		LMultiData result = biz.getStockMoveList_New(lData);
