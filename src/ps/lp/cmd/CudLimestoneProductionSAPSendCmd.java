/* ------------------------------------------------------------------------
 * @source  : CudLimestoneProductionSAPSendCmd.java
 * @desc    : Limestone Production SAP Send
 * ------------------------------------------------------------------------
 *
 * ------------------------------------------------------------------------
...
public class CudLimestoneProductionSAPSendCmd implements LGauceCommandIF {
	...
	public void execute(HttpServletRequest req, HttpServletResponse res, 
		...
		LLog.debug.write("CudLimestoneProductionSAPSendCmd ------------ START");

		GauceDataSet ds_master = gauceRequest.getGauceDataSet("IN_DS1");
		
		LData inputData = LXssCollectionUtility.getData(req);
		LLog.debug.write("CudLimestoneProductionSAPSendCmd inputData ---------------> =>\n " + inputData.toString());
		...
		LLog.debug.write("CudLimestoneProductionSAPSendCmd HEADER 자료~~~~ ---------------> =>\n " + mData.toString());

		LimestoneProductionBiz biz = new LimestoneProductionBiz();
		resultData = biz.CudLimestoneProductionSapSend(mData, loginUser);
		...
		LLog.debug.write("CudLimestoneProductionSAPSendCmd ------------ END");
