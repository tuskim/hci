/* ------------------------------------------------------------------------
 * @source  : ClinkerProductionBiz.java
 * @desc    : Clinker  등록, 조회, SAP 입고, 취소 처리
 * ------------------------------------------------------------------------
 * PROJ : 미얀마 시멘트 시스템 구축
 * ------------------------------------------------------------------------
...
public class ClinkerProductionBiz {
	
	/**
     * Clinker Production List 목록 조회
     *
     ...
     */
    public LMultiData retrieveClinkerProductionList(LData inputData) throws LException {
        LMultiData list = null;

        try {        
 
            LCommonDao dao = new LCommonDao("/ps/cp/clinkerProductionSql/retrieveClinkerProductionList", inputData);
	...
	/**
     * Clinker Production Excel List 목록 조회
     *
     * @param inputData Command로 부턴 전달받은 input LDataProtocol
     * 
     * @return LMultiData 조회된 리스트 결과.
     * @exception LException 메소드 수행시 발생한 모든 에러.
     */
    public LMultiData retrieveClinkerProductionExcelList(LData inputData) throws LException {
        LMultiData list = null;

        try {        
 
            LCommonDao dao = new LCommonDao("/ps/cp/clinkerProductionSql/retrieveClinkerProductionExcelList", inputData);
            list = dao.executeQuery();
	...
	/**
     * Use of Material for Clinker 목록 조회
     *
     * @param inputData Command로 부턴 전달받은 input LDataProtocol
     * 
     * @return LMultiData 조회된 리스트 결과.
     * @exception LException 메소드 수행시 발생한 모든 에러.
     */
    public LMultiData retrieveClinkerProductionMaterial(LData inputData) throws LException {
        LMultiData list = null;

        try {
 
            LCommonDao dao = new LCommonDao("/ps/cp/clinkerProductionSql/retrieveClinkerProductionMaterial", inputData);
            list = dao.executeQuery();
     ...
     * @exception LException 메소드 수행시 발생한 모든 에러.
     */
    public LMultiData retrieveClinkerProductionResult(LData inputData) throws LException {
        LMultiData list = null;

        try {
 
            LCommonDao dao = new LCommonDao("/ps/cp/clinkerProductionSql/retrieveClinkerProductionResult", inputData);
            list = dao.executeQuery();
    ...
    public LMultiData retrieveClinkerProductionAddRow(LData inputData) throws LException {
        ...
        try {
        	...
        	if(inputData.getString("dsType").equals("master")){
        		sql = "/ps/cp/clinkerProductionSql/retrieveClinkerProductionAddMaster";
        	}else if(inputData.getString("dsType").equals("use")){
        		sql = "/ps/cp/clinkerProductionSql/retrieveClinkerProductionAddUse";
        	}else if(inputData.getString("dsType").equals("result")){
        		sql = "/ps/cp/clinkerProductionSql/retrieveClinkerProductionAddResult";
        	}
     ...
     * @exception LException 메소드 수행시 발생한 모든 에러.
     */
    public LMultiData retrieveClinkerProductionProdDate(LData inputData) throws LException {
        LMultiData list = null;

        try {        
 
            LCommonDao dao = new LCommonDao("/ps/cp/clinkerProductionSql/retrieveClinkerProductionProdDate", inputData);
            list = dao.executeQuery();
     ...
     * @exception LException 메소드 수행시 발생한 모든 에러.
     */
    public LData cudClinkerProductionRegMod(LMultiData mDataMst, LMultiData mDataOutput, LMultiData mDataInput, LData inputData) throws LException {
        LCompoundDao dao = new LCompoundDao();
        LData result = new LData();
        int cnt = 0;
        
		LLog.debug.println("cudClinkerProduction mDataMaster--------------- =>\n " + mDataMst.toString());
		LLog.debug.println("cudClinkerProduction mDataOutput--------------- =>\n " + mDataOutput.toString());
		LLog.debug.println("cudClinkerProduction mDataInput--------------- =>\n "  + mDataInput.toString());
        ...
        try{			
			...
			for(int i=0;i<mDataMst.getDataCount();i++) {
				...
				if( "DEVON_CREATE_FILTER_VALUE".equals(cudlData.getString( "DEVON_CUD_FILTER_KEY")) ) {
					LLog.info.write("\n SQL -> /ps/cp/clinkerProductionSql/insertClinkerProdMaster -------------> \n");
		            dao.add ("/ps/cp/clinkerProductionSql/insertClinkerProdMaster", cudlData);            
					
				} else if( "DEVON_UPDATE_FILTER_VALUE".equals(cudlData.getString( "DEVON_CUD_FILTER_KEY")) ) {
					LLog.info.write("\n SQL ->  /ps/cp/clinkerProductionSql/updateClinkerProdMaster -------------> \n");
		            dao.add ("/ps/cp/clinkerProductionSql/updateClinkerProdMaster", cudlData);    
					
				} else if( "DEVON_DELETE_FILTER_VALUE".equals(cudlData.getString( "DEVON_CUD_FILTER_KEY")) ) {
					LLog.info.write("\n SQL ->  /ps/cp/clinkerProductionSql/deleteClinkerProdMaster -------------> \n");
		            dao.add ("/ps/cp/clinkerProductionSql/deleteClinkerProdMaster", cudlData);               
			...
			for(int i=0;i<mDataOutput.getDataCount();i++) {
				...
				if( "DEVON_CREATE_FILTER_VALUE".equals(cudlData.getString( "DEVON_CUD_FILTER_KEY")) ) {
					LLog.info.write("\n SQL -> /ps/cp/clinkerProductionSql/insertClinkerProdOutput -------------> \n");
		            dao.add ("/ps/cp/clinkerProductionSql/insertClinkerProdOutput", cudlData);            
					
				} else if( "DEVON_UPDATE_FILTER_VALUE".equals(cudlData.getString( "DEVON_CUD_FILTER_KEY")) ) {
					LLog.info.write("\n SQL ->  /ps/cp/clinkerProductionSql/updateClinkerProdOutput -------------> \n");
		            dao.add ("/ps/cp/clinkerProductionSql/updateClinkerProdOutput", cudlData);    
			...
			for(int i=0;i<mDataInput.getDataCount();i++) {
				...
				if( "DEVON_CREATE_FILTER_VALUE".equals(cudlData.getString( "DEVON_CUD_FILTER_KEY")) ) {
					LLog.info.write("\n SQL -> /ps/cp/clinkerProductionSql/insertClinkerProdInput -------------> \n");
		            dao.add ("/ps/cp/clinkerProductionSql/insertClinkerProdInput", cudlData);            
					
				} else if( "DEVON_UPDATE_FILTER_VALUE".equals(cudlData.getString( "DEVON_CUD_FILTER_KEY")) ) {
					LLog.info.write("\n SQL ->  /ps/cp/clinkerProductionSql/updateClinkerProdInput -------------> \n");
		            dao.add ("/ps/cp/clinkerProductionSql/updateClinkerProdInput", cudlData);    
     ...
     * @exception LException 메소드 수행시 발생한 모든 에러.
     */   
    public LData cudClinkerProductionDelete(LMultiData mDataMst, LData inputData) throws LException {
        LCompoundDao dao = new LCompoundDao();
        LData result = new LData();
        int cnt = 0;
     ...
     * @exception LException 메소드 수행시 발생한 모든 에러.
     */   
    public LData cudClinkerProductionApproval(LMultiData mDataMst, LData inputData) throws LException {
        LCompoundDao dao = new LCompoundDao();
        LData result = new LData();
        int cnt = 0;
        ...
        try{			
			...
			for(int i=0;i<mDataMst.getDataCount();i++) {
				...
				if( "DEVON_DELETE_FILTER_VALUE".equals(cudlData.getString( "DEVON_CUD_FILTER_KEY")) ) {
					LLog.info.write("\n SQL ->  /ps/cp/clinkerProductionSql/approvalClinkerProdMaster -------------> \n");
		            dao.add ("/ps/cp/clinkerProductionSql/approvalClinkerProdMaster", cudlData);               
				} 
			}
     ...
     * @exception LException 메소드 수행시 발생한 모든 에러.
     */   
    public LData cudClinkerProductionReject(LMultiData mDataMst, LData inputData) throws LException {
        LCompoundDao dao = new LCompoundDao();
        LData result = new LData();
        int cnt = 0;
        ...
        try{			
			...
			for(int i=0;i<mDataMst.getDataCount();i++) {
				...
				if( "DEVON_DELETE_FILTER_VALUE".equals(cudlData.getString( "DEVON_CUD_FILTER_KEY")) ) {
					LLog.info.write("\n SQL ->  /ps/cp/clinkerProductionSql/rejectClinkerProdMaster -------------> \n");
		            dao.add ("/ps/cp/clinkerProductionSql/rejectClinkerProdMaster", cudlData);               
				} 
			}
	...
	/**
     * SAP SEND 전에 이전 공정에서 동일 생산일자 기준 SAP전송이 완료(STATUS:04) 상태 조회
     *
     * @param inputData Command로 부턴 전달받은 input LDataProtocol
     * 
     * @return LMultiData 조회된 리스트 결과.
     * @exception LException 메소드 수행시 발생한 모든 에러.
     */
    public LMultiData checkClinkerBeforeProductionStatus(LData inputData) throws LException {
        LMultiData list = null;

        try {        
 
            LCommonDao dao = new LCommonDao("/ps/cp/clinkerProductionSql/checkClinkerBeforeProductionStatus", inputData);
            list = dao.executeQuery();
     ...
     * @exception LException 메소드 수행시 발생한 모든 에러.
     */
    public LData CudClinkerProductionSapSend(LMultiData headerData, LData loginUser) throws LException {
    	
    	LLog.info.write("\n Biz > CudClinkerProductionSapSend() ------------- Start \n");
    		...
    		for(int i=0; i<headerData.getDataCount(); i++) {
    			...
    			if (headerData.getString("status", i).equals("01")) { //신규 전송 건
    				...
    				//전송시 상태 01(Progress)인지 재확인	
    				 LData checkStatus = statusDao.executeQueryForSingle( "/ps/cp/clinkerProductionSql/checkSendingStatus" , header );
    				 LLog.info.write("SAP 전송할때의 status----->: " +checkStatus.getString("status"));								 
    				 if(checkStatus.getString("status").equals("01")){

    					LCommonDao commonDao = new LCommonDao( "/ps/cp/clinkerProductionSql/cudClinkerProductionOutputSapSend" , header );
    					bodyData1 = commonDao.executeQuery();
    					LCommonDao commonDao2 = new LCommonDao( "/ps/cp/clinkerProductionSql/cudClinkerProductionIputSapSend" , header );
    					bodyData2 = commonDao2.executeQuery();
    					
    					resultMsg = gmm11.GMM11_out(header, bodyData1, bodyData2);
    					
    					 //전송시 상태 02(sent)으로 변경
    					//statusDao = new LCommonDao( "/ps/cp/clinkerProductionSql/cudClinkerProductionSendingStatus" , header );
    					...
    					if (resultMsg.getString("returnType").equals("00")) {
    						dao.add ("/ps/cp/clinkerProductionSql/cudClinkerProductionAfterSapSuccess", header );
    					}else{
    						dao.add ("/ps/cp/clinkerProductionSql/cudClinkerProductionAfterSapError", header ); 
    					}
    			...
    			if (!headerData.getString("status", i).equals("00") && !headerData.getString("status", i).equals("01")) { //신규 전송 건
    				...
    				//취소시 상태 01(Progress) 재확인
    				 ...
    				 if(!checkStatus.getString("status").equals("00") && !checkStatus.getString("status").equals("01")){
    					...
    					if (resultMsg.getString("returnType").equals("00")) {
    						dao.add ("/ps/cp/clinkerProductionSql/cudClinkerProductionAfterSapCancel", header );
    					}else{
    						dao.add ("/ps/cp/clinkerProductionSql/cudClinkerProductionAfterSapError", header ); 
    					}
