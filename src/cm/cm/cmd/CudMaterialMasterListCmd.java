...
public class CudMaterialMasterListCmd implements LGauceCommandIF {
    ...
    public void execute(HttpServletRequest req, HttpServletResponse res, GauceRequest gauceRequest, GauceResponse gauceResponse) throws Exception {
    	...
    	LLog.info.write("\n CudMaterialMasterListCmd --> Start \n");	
		...
		if( dsGrid != null ) {
			...
			biz.cudMaterialMasterList( lmData, req );
		} 
		...
		LGauceConverter.extractToGauceDataSet( biz.retrieveAccountManage(lData), gauDsRes );
		gauDsRes.flush();
    
		LLog.info.write("\n CudMaterialMasterListCmd --> End \n");
    }
	...
	String companyCd = loginUser.getString("companyCd");
    	for (int i = 0; i < rowCount && i < 5000; i++) {
			lmData.add("userId", userId);
			lmData.add("companyCd", companyCd);
		}
    }
}