...
public class CudBlockHrInfoMgntCmd implements LGauceCommandIF {
    ...
    public void execute(HttpServletRequest req, HttpServletResponse res, GauceRequest gauceRequest, GauceResponse gauceResponse) throws Exception {
		...
		if( dsGrid != null ) {
			...
			biz.cudBlockHrInfoMgnt( lmData, req );
		} 
		...
		LGauceConverter.extractToGauceDataSet( biz.retrieveBlockHrInfoMgnt(lData), gauDsRes );
		gauDsRes.flush();
    
		LLog.info.write("\n CudBlockHrInfoMgntCmd --> End \n");
    }
    ...
    private void setUserID(HttpServletRequest req, LMultiData lmData, String keyName) {
    	...
    	String userId = ((LData) req.getSession().getAttribute("loginUser")).getString("userId");
